        ...
        layui.use('upload', function(upload){
          ...
          upload.render({
            ...
            ,done: function(res){
              ...
              if(res.code == 0){
                res.data = res.data || {};
                  var time = new Date().getTime().toString();//获取毫秒级时间戳
                  var resultPath = res.url.substring(26, res.url.length);//截取路径
                  var token = md5(resultPath + "FastDFS1234567890" + time.substring(0, time.length - 3));//MD5加密
                  var resultPath = res.url + "?token=" + token + "&ts=" + time.substring(0, time.length - 3);//请求图片的路径
                  var checkPath=resultPath.replace("http://10.63.22.23","http://61.185.20.20:14100");
                insertInline.call(iframeWin, 'img', {
                  src: checkPath
                  ,alt: res.url
                }, range);

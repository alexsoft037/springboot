...
(function ($) {
    ...
    var selectTool = {
        ...
        renderData: function (R, $grid, cyProps) {
            ...
            var _level = '';
            if( cyProps.idtemp){
                _idtemp = cyProps.idtemp;
                _level = cyProps.level;
            }
            ...
            if(_value!=""&&_values != undefined){
                ...
                for(var j=0;j<_values.length;j++){
                    if(_values[j]!="" && _values[j] != " "){
                        ...
                        if(curr_data){
                            var _div = '<div id="div_' + _values[j] +'_'+_idtemp+ '" class="layui-input-inline" levelNum="'+_level+'"></div>';
                            $(_grid).parent().append(_div);
                            var _select = '<select id="select_' + _values[j] +'_'+_idtemp+  '" name="'+cyProps.name+'" lay-search lay-filter="link" ></select>';
                            $("#div_" + _values[j]+'_'+_idtemp).append(_select);
                            $("#select_" + _values[j]+'_'+_idtemp).append('<option value="">请选择</option>');
                            for (var i = 0; i < curr_data.length; i++) {
                                ...
                                $("#select_" + _values[j] +'_'+_idtemp).append(_option);
                            }
                        }
                        }
            ...
            layui.use(['form'], function () {
                ...
                form.on('select(link)', function (data) {
                    ...
                    var name = '';
                    if(data.elem.id){
                        var idtemp_t = data.elem.id;
                        var idtemp_ta = idtemp_t.split('_');
                        if(idtemp_ta.length ==3){
                            idtemp = idtemp_ta[2];
                        }
                    }
                    if(data.elem.name){
                    ...
                    var cc1 = $("select[name='"+name+"']").length;
                    var level = 0;
                    if($(_grid).attr("levelNum")){
                        level = $(_grid).attr("levelNum") -1+1;
                    }

                    if (parentId != "" && parentId != " ") {
                        if(level >0 && cc1 >= level){
                            return;
                        }
                        //获取所有下级地区
                        ...
                        if (R.data&&R.data.length > 0) {
                            var _div = '<div id="div_' + data.value +'_'+ idtemp + '" class="layui-input-inline" levelNum="'+_level+'"></div>';
                            $(_grid).after(_div);
                            var _select = '<select id="select_' + data.value +'_'+ idtemp + '" name="'+ name +'" lay-search lay-filter="link" ></select>';
                            $("#div_" + data.value +'_'+ idtemp).append(_select);
                            $("#select_" + data.value +'_'+ idtemp).append('<option value="">请选择</option>');
                            for (var i = 0; i < R.data.length; i++) {
                                var _option = '<option value="' + R.data[i].code + '">' + R.data[i].value + '</option>';
                                $("#select_" + data.value +'_'+ idtemp).append(_option);
                            }
                        }

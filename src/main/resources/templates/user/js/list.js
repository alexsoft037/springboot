...
var Render = {
    ...
    edit:function(rowdata,renderData){
        var btn=' <button  onclick="editOne(\''+"/sys/user/edit"+'\',\''+rowdata.userId+'\')" class="layui-btn layui-btn-mini">修改</button>';
        return btn;
    },
    /**
     * @param rowdata    行数据
    ...
    delete:function(rowdata,renderData){
        var btn=' <button  onclick="deleteOne(\''+"删除"+'\',\''+"/sys/user/delete"+'\',\''+rowdata.userId+'\')" class="layui-btn layui-btn-mini layui-btn-delete">删除</button>';
        return btn;
    },
    /**
     * @param rowdata    行数据
    ...
    state:function(rowdata,renderData){
        if(rowdata.status=='0'){
            return' <button onclick="updateStateOne(\''+"启用"+'\',\''+"/sys/user/enable"+'\',\''+rowdata.userId+'\')"' +
                '  class="layui-btn layui-btn-mini layui-btn-green">启用</button>';
        }
        if(rowdata.status=='1'){
            return' <button  onclick="updateStateOne(\''+"禁用"+'\',\''+"/sys/user/limit"+'\',\''+rowdata.userId+'\')" ' +

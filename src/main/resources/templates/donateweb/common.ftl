...
<div class="layui-form-item" id="author-box" style="display: none;">${(model.bucket)!""}
    ...
    <div class="layui-input-normal">
        <input type="text" name="author" maxlength="256" value="${(model.author)!""}" placeholder="请输入" class="layui-input">
    </div>
</div>

<input type="hidden" id="author" value="${(model.createBy)!""}">


<div class="layui-form-item">${(model.bucket)!""}
    <label class="layui-form-label"><span class="span_must">*</span>创建时间</label>
    <div class="layui-input-normal">
        <input type="text" name="createAt" id="createAt" maxlength="25" placeholder="yyyy-MM-dd HH:mm:ss" lay-verify="required" value="${(model.createAt?string["yyyy-MM-dd HH:mm:ss"])!""}" lay-verify="datetime" class="layui-input">
    </div>
</div>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        if (author == "" || author == null || author.length < 1){

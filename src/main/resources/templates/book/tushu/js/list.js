...
var Render = {
    ...
    info:function(rowdata,renderData){
        var btn=' <button  onclick="detailOne(\''+"/bookTuShu/info"+'\',\''+rowdata.bookId+'\')" class="layui-btn layui-btn-mini">详情</button>';
        return btn;
    },
    /**
     * @param rowdata    行数据
    ...
    edit:function(rowdata,renderData){
        var btn=' <button  onclick="editOne(\''+"/bookTuShu/edit"+'\',\''+rowdata.bookId+'\')" class="layui-btn layui-btn-mini">修改</button>';
        return btn;
    },
    /**
     * @param rowdata    行数据
    ...
    delete:function(rowdata,renderData){
        var btn=' <button  onclick="deleteOne(\''+"删除"+'\',\''+"/bookTuShu/delete"+'\',\''+rowdata.bookId+'\')" class="layui-btn layui-btn-mini layui-btn-delete">删除</button>';
        return btn;
    },
    /**
     * @param rowdata    行数据
    ...
    state:function(rowdata,renderData){
        if(rowdata.state=='0'){
            return' <button onclick="updateStateOne(\''+"启用"+'\',\''+"/bookTuShu/enable"+'\',\''+rowdata.bookId+'\')"' +
                '  class="layui-btn layui-btn-mini layui-btn-green">启用</button>';
        }
        if(rowdata.state=='1'){
            return' <button  onclick="updateStateOne(\''+"禁用"+'\',\''+"/bookTuShu/limit"+'\',\''+rowdata.bookId+'\')" ' +

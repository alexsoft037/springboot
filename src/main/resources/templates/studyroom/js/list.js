...
var Render = {
    ...
    info:function(rowdata,renderData){
        var btn=' <button  onclick="detailOne(\''+"/studyroom/info"+'\',\''+rowdata.roomId+'\')" class="layui-btn layui-btn-mini">详情</button>';
        return btn;
    },
    /**
     * @param rowdata    行数据
    ...
    edit:function(rowdata,renderData){
        var btn=' <button  onclick="editOne(\''+"/studyroom/edit"+'\',\''+rowdata.roomId+'\')" class="layui-btn layui-btn-mini">修改</button>';
        return btn;
    },
    /**
     * @param rowdata    行数据
    ...
    delete:function(rowdata,renderData){
        var btn=' <button  onclick="deleteOne(\''+"删除"+'\',\''+"/studyroom/delete"+'\',\''+rowdata.roomId+'\')" class="layui-btn layui-btn-mini layui-btn-delete">删除</button>';
        return btn;
    },
    /**
     * @param rowdata    行数据
    ...
    state:function(rowdata,renderData){
        if(rowdata.state=='0'){
            return' <button onclick="updateStateOne(\''+"启用"+'\',\''+"/studyroom/enable"+'\',\''+rowdata.roomId+'\')"' +
                '  class="layui-btn layui-btn-mini layui-btn-green">启用</button>';
        }
        if(rowdata.state=='1'){
            return' <button  onclick="updateStateOne(\''+"禁用"+'\',\''+"/studyroom/limit"+'\',\''+rowdata.roomId+'\')" ' +

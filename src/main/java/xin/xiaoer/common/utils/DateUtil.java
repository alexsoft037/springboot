...
public class DateUtil {
	...
	private static SimpleDateFormat SDF_Y_Sp_M = new SimpleDateFormat("MM");
	...
	public static String getYear() {
		return SDF_Y.format(new Date());
	}
	public static String getMonth() {
		return SDF_M.format(new Date());
	}
	public static String getYearSpMonth() {
		return SDF_Y.format(new Date());
	}
	/**
	 * @author chenyi
...
//			Date dt2 = SDF_Y_M_D.parse(DATE2);
	 ...
	 * @date
	 */
	public static Date changeYMDDate(Date date) {
		String dateStr = SDF_Y_M_D.format(date);
		Date date2 = null;
		try {
			date2 = SDF_Y_M_D.parse(dateStr);
		} catch (ParseException e) {
			e.printStackTrace();
		}
		return date2;
	...
	public static Date getToday() {
		String dateStr = SDF_Y_M_D.format(new Date());
		Date date2 = null;
		try {
			date2 = SDF_Y_M_D.parse(dateStr);
		} catch (ParseException e) {
	...
	public static Date str2SDF_YMDHMS(String dateStr){
		try {
	...
	public static Date str2SDFYMD(String dateStr){
		try {
			if (dateStr != null && !dateStr.equals("")){
				return SDF_Y_M_D.parse(dateStr);
			} else {
